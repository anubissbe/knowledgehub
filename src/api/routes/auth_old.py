...
async def setup_initial_api_key(
    ...
    # Sanitize input
    name = InputSanitizer.sanitize_text(request.name, max_length=255, allow_html=False)
...
async def create_api_key(
    ...
    if "admin" not in current_user.get("permissions", []):
        raise HTTPException(
            status_code=403,
            detail="Admin permissions required to create API keys"
        )
...
async def list_api_keys(
    ...
    if "admin" not in current_user.get("permissions", []):
        raise HTTPException(
            status_code=403,
            detail="Admin permissions required to list API keys"
        )
...
async def revoke_api_key(
    ...
    if "admin" not in current_user.get("permissions", []):
        raise HTTPException(
            status_code=403,
            detail="Admin permissions required to revoke API keys"
        )
