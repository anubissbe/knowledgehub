...
class ValidationMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app, validation_level: ValidationLevel = ValidationLevel.MODERATE):
        ...
        self.skip_validation = {
            ...
            '/api/redoc',
            '/api/openapi.json',
            '/api/v1/admin/system/command',  # Skip validation for admin commands
            '/api/v1/diagrams/',  # Skip validation for diagram operations
            '/api/performance/',  # Skip validation for performance endpoints
        ...
        self.validation_rules = {
            ...
            '/api/patterns/analyze': {
                'code': {'content_type': ContentType.TEXT, 'required': True, 'max_length': 1000000, 'skip_security_patterns': True},
                'language': {'content_type': ContentType.TEXT, 'required': False, 'max_length': 50},
                'session_id': {'content_type': ContentType.UUID, 'required': False},
                'file_path': {'content_type': ContentType.FILENAME, 'required': False},
                'metadata': {'content_type': ContentType.JSON, 'required': False}
            },
            '/api/auth/': {
    ...
    async def _validate_request_body(self, request: Request, source_ip: str, user_agent: str):
        ...
        try:
            ...
            # IMPORTANT: Set the body back on the request so it can be read again by the endpoint
